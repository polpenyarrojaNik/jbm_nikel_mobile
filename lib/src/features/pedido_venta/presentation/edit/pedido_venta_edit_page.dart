...
class PedidoVentaEditPage extends ConsumerStatefulWidget {
  PedidoVentaEditPage({
    ...
    this.addLineaDesdeArticulo,
       ...
       isEdit = (pedidoAppId != null || pedidoId != null);
...
class _PedidoVentaEditPageState extends ConsumerState<PedidoVentaEditPage> {
  ...
  void onSavedOrSavedErrorMessage({Object? error}) async {
    if (error != null) {
      unawaited(
        context.showErrorBar(
          duration: const Duration(seconds: 5),
...
class PedidoVentaEditForm extends ConsumerWidget {
  ...
  void continued(BuildContext context, int currentStep, WidgetRef ref) {
    ...
    switch (currentStep) {
      case 0:
        ...
        break;
      case 1:
        selectClienteDireccionValidate(ref);
        break;
      case 2:
  ...
  void selectClienteDireccionValidate(WidgetRef ref) {
    ref
        .read(pedidoVentaEditPageControllerProvider(pedidoLocalParam).notifier)
        .navigateToNextStep();
  }
  ...
  void remarksValidate(BuildContext context, WidgetRef ref) {
    if (!oferta || oferta && ofertaFechaHasta != null) {
      if (pedidoCliente == null ||
        ...
        showToast(
          S.of(context).pedido_edit_pedidoEdit_pedidoClienteMax50,
          context,
        );
      }
    } else {
  ...
  List<IconStep> getSteps() {
    return [
      IconStep(
        ...
        state: (() {
          if (currentStep < 0) {
            return IconStepState.disabled;
          }
          if (currentStep == 0) {
            return IconStepState.editing;
          }
          return IconStepState.complete;
        })(),
